...
| **A2** Ingestão | `src/ingest.py`, `tests/test_ingest.py` | unit: splitter 1000/150 e pipeline com doubles | 3→4 | A1, DEC-01/06 |
| **B1** Busca | `src/search.py`, `tests/test_search.py` | unit: `similarity_search_with_score(k=10)` com store fake | 3→4 | A2 |
| **B2** Chat/CLI | `src/chat.py`, `tests/test_chat.py` | unit: prompt byte-a-byte + fallback exato (LLM fake) | 3→4 | B1 |
